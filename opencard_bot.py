...
# =============================================================================
# AI ГЕНЕРАЦИЯ: LEONARDO + RUNPOD FACE SWAP
# =============================================================================
...
async def generate_postcard(photo_path, theme, style, scene, orientation="vertical"):
    """
    Полный пайплайн генерации открытки с face swap
    
    1. Leonardo Phoenix → генерация шаблона
    2. RunPod Face Swap → замена лиц
    3. Постобработка (рамка, эффекты, текст)
    4. Загрузка на Telegraph
    """
    logger.info(f"[PIPELINE] Starting: {theme}/{style}/{scene}")
    
    try:
        # ШАГ 1: Генерируем шаблон через Leonardo
        logger.info("[STEP 1/4] Generating template via Leonardo...")
        template_path = await generate_template_leonardo(theme, style, scene, orientation)
        
        if not template_path:
            logger.error("Template generation failed")
            return None
        
        logger.info(f"[STEP 1/4] ✓ Template ready")
        
        # ШАГ 2: Face swap через RunPod
        logger.info("[STEP 2/4] Face swap via RunPod...")
        swapped_path = await faceswap_runpod(template_path, photo_path)
        
        if not swapped_path:
            logger.warning("Face swap failed, using template without face swap")
            swapped_path = template_path
        
        logger.info(f"[STEP 2/4] ✓ Face swap complete")
        
        # ШАГ 3: Постобработка
        logger.info("[STEP 3/4] Post-processing...")
        
        img = Image.open(swapped_path)
        ...
        final_path = f"/tmp/final_{uuid.uuid4().hex}.png"
