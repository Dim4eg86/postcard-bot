...
# УСИЛЕННЫЙ ПРОМПТ: явно просим лицо человека
RETRO_PROMPT = (
    "Close-up portrait of a happy person in Soviet 1960s winter outfit, "
    "vintage Soviet New Year postcard style, gouache painting, "
    "clear face features, nostalgic atmosphere, masterpiece illustration"
)
    ...
    try:
        ...
        for _ in range(80):
            ...
            if status_res.get("status") == "COMPLETED":
                ...
                img_data = output.get("image_url") or output.get("image") if isinstance(output, dict) else output
                if img_data and str(img_data).startswith("http"): return requests.get(img_data).content
                return base64.b64decode(str(img_data).split(",")[-1])
...
def finalize_image(image_bytes):
    ...
    img = ImageOps.expand(img, border=(30, 30, 30, 100), fill='#FDFBF5')
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    m = await update.message.reply_text("❄️ Рисую открытку с вашим лицом...")
    try:
        ...
        await m.edit_text("👤 Накладываю ваше лицо (этот шаг критичный)...")
        swapped = await runpod_face_swap(bg_url, user_img)
        
        if swapped:
            final = finalize_image(swapped)
            await update.message.reply_photo(final, caption="🎉 Ура! Ваша открытка готова.")
            await m.delete()
        else:
            await m.edit_text("❌ Нейросеть не нашла лицо на фоне. Попробуйте еще раз, она нарисует по-другому!")
...
def main():
    ...
    app.add_handler(CommandHandler("start", lambda u, c: u.message.reply_text("Пришлите фото (селфи)!")))
