...
def finalize_card(image_bytes):
    """Добавление ретро-рамки и текста"""
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if res_url:
            final_card = finalize_card(requests.get(res_url).content)
            await update.message.reply_photo(final_card, caption="🎄 Ваша открытка готова!")
            await m.delete()
        else:
            await m.edit_text("❌ Не удалось создать образ. Попробуйте другое фото.")
    except Exception as e:
