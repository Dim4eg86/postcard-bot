...
async def get_init_image_id(image_bytes):
    ...
    try:
        ...
        # Увеличили базовое ожидание до 20 секунд
...
async def generate_transformed_image(init_image_id):
    ...
    # Цикл повторных попыток запуска генерации (если ID еще не готов)
    gen_id = None
    for attempt in range(5):
        try:
            ...
            if 'invalid init generation image id' in error_msg:
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = await update.message.reply_text("❄️ Начинаю превращение (это займет около 1-2 минут)...")
    try:
        ...
        await msg.edit_text("🎨 Рисую ваш образ (ожидание очереди)...")
        ...
        if image_url:
            ...
            await msg.edit_text("❌ Ошибка: Leonardo не смог обработать фото. Попробуйте еще раз.")
    except Exception as e:
        logger.error(f"Handler error: {e}")
        await msg.edit_text("❌ Ошибка бота.")
